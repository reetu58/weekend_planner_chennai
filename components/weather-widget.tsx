  ...
  if (c.includes('rain') || c.includes('shower') || c.includes('drizzle')) {
    return (
      <svg viewBox="0 0 64 64" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <line x1="24" y1="52" x2="20" y2="60" stroke="#3B82F6" strokeWidth="2.5" strokeLinecap="round"/>
        <line x1="32" y1="52" x2="28" y2="60" stroke="#3B82F6" strokeWidth="2.5" strokeLinecap="round"/>
        <line x1="40" y1="52" x2="36" y2="60" stroke="#3B82F6" strokeWidth="2.5" strokeLinecap="round"/>
      </svg>
    );
  }
  ...
  if (c.includes('thunder') || c.includes('storm')) {
    return (
      <svg viewBox="0 0 64 64" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M33 34l-6 12h5l-3 8 10-14h-6l4-6h-4z" fill="#F59E0B" stroke="#F59E0B" strokeWidth="0.5" strokeLinejoin="round"/>
      </svg>
    );
  }
  return (
...
function getWeatherAccent(condition: string, temp: number): { text: string; bg: string } {
  const c = condition.toLowerCase();
  if (c.includes('rain') || c.includes('shower') || c.includes('drizzle'))
  ...
  if (c.includes('cloud') || c.includes('overcast') || c.includes('fog') || c.includes('mist') || c.includes('haze'))
    return { text: 'text-slate-500', bg: 'bg-slate-50' };
  if (temp >= 38)
    return { text: 'text-red-600', bg: 'bg-red-50' };
  return { text: 'text-amber-500', bg: 'bg-amber-50' };
}
...
export default function WeatherWidget() {
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5 w-full h-full min-h-[140px] flex items-center justify-center">
        <span className="w-5 h-5 border-2 border-slate-200 border-t-slate-400 rounded-full animate-spin block" />
      </div>
    );
  }
    ...
    <div className="relative rounded-2xl border border-slate-200 bg-white shadow-sm p-5 overflow-hidden">
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`text-[10px] font-bold uppercase tracking-wider ${weather.isLive ? 'text-emerald-600' : 'text-slate-400'}`}>
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            <div className="flex items-start">
              <span className="text-5xl font-black text-[#0F172A] leading-none">{weather.temperature}</span>
              <span className="text-xl text-slate-400 font-light mt-1 ml-0.5">°C</span>
            </div>
            <p className="text-sm text-slate-500 mt-1">{weather.condition}</p>
          </div>
        </div>

        <div className="grid grid-cols-3 gap-2">
          <div className="flex flex-col items-center gap-1 p-2.5 rounded-xl bg-white border border-slate-200">
            ...
            <span className="text-sm font-bold text-[#0F172A]">{weather.humidity}%</span>
            <span className="text-[10px] text-slate-500 uppercase tracking-wide">Humidity</span>
          </div>

          <div className={`flex flex-col items-center gap-1 p-2.5 rounded-xl border ${weather.rainChance > 60 ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200'}`}>
            <svg className={`w-4 h-4 ${weather.rainChance > 60 ? 'text-blue-500' : 'text-slate-400'}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <span className={`text-sm font-bold ${weather.rainChance > 60 ? 'text-blue-600' : 'text-[#0F172A]'}`}>{weather.rainChance}%</span>
          ...
          <div className="flex flex-col items-center gap-1 p-2.5 rounded-xl bg-white border border-slate-200">
            ...
            <span className="text-sm font-bold text-[#0F172A]">{weather.windSpeed}<span className="text-xs font-normal text-slate-400"> km/h</span></span>
            <span className="text-[10px] text-slate-500 uppercase tracking-wide">Wind</span>
          </div>
        </div>

        {(weather.rainChance > 60 || weather.temperature > 38) && (
          <div className={`mt-3 px-3 py-2 rounded-xl text-xs font-medium flex items-center gap-2 ${weather.temperature > 38 ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-blue-50 text-blue-600 border border-blue-200'}`}>
